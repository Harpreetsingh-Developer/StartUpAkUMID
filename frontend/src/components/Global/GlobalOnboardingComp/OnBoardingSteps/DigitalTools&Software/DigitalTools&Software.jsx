...
import "./DigitalTools&Software.scss";

const DigitalToolsSoftware = () => {
  ...
  const {
    ...
    defaultValues: formData.digitalToolsSoftware || {}
  });

  useEffect(() => {
    const subscription = watch((value) => {
      updateStepData("digitalToolsSoftware", value);
    });
    return () => subscription.unsubscribe();
  }, [watch]);

  const onSubmit = (data) => {
    updateStepData("digitalToolsSoftware", data);
    console.log("Saved Digital Tools & Software:", data);
  };

  return (
    <form className="digital-tools-software-form" onSubmit={handleSubmit(onSubmit)}>
      <h3>Digital Tools & Software</h3>
      <hr />
      <div className="grid">
        <div className="form-group">
          <label htmlFor="toolName">Tool/Software Name</label>
          <input type="text" id="toolName" {...register("toolName", { required: true })} />
          {errors.toolName && <span className="error">Required</span>}
        </div>
...
export default DigitalToolsSoftware;
