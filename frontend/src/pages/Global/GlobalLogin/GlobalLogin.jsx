...
const GlobalUserLogin = () => {
  ...
  const checkAuthStatus = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (data.data.user.role === 'global') {
          localStorage.setItem('role', 'global');
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/auth/globallogin`, {
        method: 'POST',
        credentials: 'include',
        headers: {
          'Content-Type': 'application/json',
  ...
  // Choose an icon from constants for the admin login (adjust as needed)
  const adminIcon = icons.Global;
